...
@torch.no_grad()
def a_quant_per_token(
    x_fp: torch.Tensor, a_bits: int=16, group_size: int=-1
):
    """
    Symmetric per-group activation quantization.

    :param x_fp: input tensor to be quantized
    :param a_bits: activation bit-width
...
def q_quant_per_head(
    x_fp: torch.Tensor, q_bits: int=16
):
    """
    Symmetric per-group activation quantization.
    ...
    scale = rmax / qmax
    scale = scale.clamp_(min=1e-6)
    x_scaled = (x_fp / scale).abs()

    x_dq_sign = torch.sign(x_fp)
    x_dq_exp  = (x_scaled + (x_scaled == 0).type(x_scaled.dtype)).log2().floor().clamp_(min=-6)
    x_dq_man  = torch.round(x_scaled / 2**x_dq_exp * 2**3) / 2**3
...
def k_quant_per_token(
    x_fp: torch.Tensor, k_bits: int=4, group_size: int=128, 
    ...
    k_quant_post_rope: Optional[bool]=False
):
    """
    Asymmetric per-token key quantization.

    :param x_fp: input tensor to be quantized
    :param k_bits: quantization bit-width
    :param group_size: quantization group size
    ...
    batch, num_head, seq_len, h_dim = x_fp.shape
    x_fp_new = (
        x_fp_new.permute(0, 2, 1, 3).view(batch, seq_len, num_head * h_dim)
    )
...
def v_quant_per_token(
    x_fp: torch.Tensor, v_bits: int=4, group_size: int=128
):
    """
    Asymmetric per-token value quantization.

    :param x_fp: input tensor to be quantized
    :param v_bits: quantization bit-width
    :param group_size: quantization group size
    """
    batch, num_head, seq_len, h_dim = x_fp.shape
    if v_bits >= 16:
    ...
    scale_fp = scale_fp.view(batch, seq_len, num_head, h_dim // group_size).permute(0, 2, 1, 3)
    x_q = x_q.view(batch, seq_len, num_head, h_dim).permute(0, 2, 1, 3)
...
def p_quant_per_block(
    x_fp: torch.Tensor, p_bits: int=8
):
    """
    Asymmetric per-token value quantization.

    :param x_fp: input tensor to be quantized
    :param p_bits: quantization bit-width
    """
    assert p_bits in [8, 12, 16], \
        f'Invalid precision \"{p_bits}\" provided for attention score. Allowed precisions are {{8, 12, 16}}'
    ...
    #     # x_fp_lsb = x_fp_tmp.bitwise_and(3)
    #     # roundup_mask = x_fp_lsb.gt(1)
    #     # x_dq[roundup_mask] = x_dq[roundup_mask] + 4
...
def a_quant_function(
    a_fp, quant_config
):
    a_bits = quant_config.a_bits
    a_group_size = quant_config.a_group_size
    a_dq = a_quant_per_token(a_fp, a_bits, a_group_size)

    return a_dq


def q_quant_function(
    q_fp, quant_config
):
    q_bits = quant_config.q_bits
    q_dq = q_quant_per_head(q_fp, q_bits)
...
def k_quant_function(
    ...
    if kv_quant_method == "KTVT": # key per-token, value per-token
        k_dq = k_quant_per_token(
            ...
            k_quant_post_rope=quant_config.k_quant_post_rope
        )
...
def quant_matmul_pv(
    ...
    v_int = v_int.transpose(2, 3) # batch, num_head, num_groups_per_head, kv_seq_len, group_size
    ...
    attn_output = attn_output.transpose(2, 3).view(batch, num_head, q_seq_len, h_dim) # batch, num_head, q_seq_len, h_dim
