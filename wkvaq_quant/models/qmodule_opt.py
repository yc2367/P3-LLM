...
from quantize.quantizer import a_quant_function, k_quant_function, v_quant_function, quant_matmul_pv
...
class QuantOPTAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert not quant_config.kv_quant_post_attn, \
            f"For OPT, the KV-cache must be quantized before performing self-attention"
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_states = self._shape(self.k_proj(hidden_states_quant), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states_quant), -1, bsz)
        ...
        if value_states_quant_int is None:
            attn_output = torch.matmul(attn_probs, value_states_float) 
        elif value_states_float is None:
            attn_output = quant_matmul_pv(
                attn_probs, 
                value_states_quant_int, value_states_quant_scale,
                self.quant_config,  # is_prefill=True
            ) 
        else:
            value_states_float_len = value_states_float.shape[-2]
            attn_probs_quant = attn_probs[..., :-value_states_float_len]
            attn_probs_float = attn_probs[..., -value_states_float_len:]
            attn_output_quant = quant_matmul_pv(
                attn_probs_quant, 
                value_states_quant_int, value_states_quant_scale,
                self.quant_config,  # is_prefill=True
            ) 
            attn_output_float = torch.matmul(attn_probs_float, value_states_float) 
        ...
        attn_output = self.out_proj(attn_output_quant)
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig, quant_config: QuantConfig):
        super().__init__()
        self.embed_dim = config.hidden_size

        self.self_attn = QuantOPTAttention(config=config, quant_config=quant_config, is_decoder=True)
    ...
    def forward(
        ...
        #NOTE (Yuzong): activation quantization
        hidden_states = a_quant_function(hidden_states, self.quant_config)
...
class QuantOPTDecoder(OPTPreTrainedModel):
    """
    ...
    def __init__(self, config: OPTConfig, quant_config: QuantConfig):
        ...
        self.layers = nn.ModuleList([OPTDecoderLayer(config, quant_config) for _ in range(config.num_hidden_layers)])
...
class QuantOPTModel(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig, quant_config: QuantConfig):
        super().__init__(config)
        self.decoder = QuantOPTDecoder(config, quant_config)
        # Initialize weights and apply final processing
        self.post_init()
...
class QuantOPTForCausalLM(OPTPreTrainedModel):
    ...
    def __init__(self, config, quant_config: QuantConfig):
        super().__init__(config)
        self.model = QuantOPTModel(config, quant_config)
